# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 08:48:52 2025
...
file_path = r'C:\Users\Steven.Li\OneDrive - MMC\Desktop\Power System Modelling\Illinois Energy Storage\UCS Analysis'
input_file = os.path.join(file_path, "data/heatrates.csv")
#%%

zz issue with firewall
...
if __name__ == "__main__":
    # config file options
    state_abbr = 'IL' #snakemake.config['state_abbr']
    time_res = 1 #snakemake.config['time_res']
    year = 2023 #snakemake.config['fuel_cost_year']

    eia_923m_df = retrieve_eia923m_pudl(state=state_abbr)

    fuel_cost_df = fuel_cost_pivot(eia_923m_df)
    heatrates = pd.read_csv(input_file,
                            parse_dates=True,
                            index_col='Year')
    ...
    fuel_cost_df.to_csv(os.path.join(file_path, "data/thermal_fuel_costs.csv")) #snakemake.output.fuel_costs)
    fuel_cost_resampled = prepare_cost_timeseries(fuel_cost_df, time_res)
    fuel_cost_resampled.to_csv(os.path.join(file_path, "data/thermal_fuel_cost_ts.csv")) #snakemake.output.fuel_cost_timeseries)